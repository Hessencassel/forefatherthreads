...
export default function ImageGallery({ product, selectedColor }: ImageGalleryProps) {
  ...
  const imageCount = hasRealImages ? colorImages.length : product.imageCount;
    ...
    <div className="flex flex-col gap-3">
      ...
      {hasRealImages ? (
        <div className="aspect-[4/5] w-full relative overflow-hidden bg-white">
          <img
            src={colorImages[activeIndex]}
            alt={`${product.name} in ${selectedColor.name} — view ${activeIndex + 1} of ${imageCount}`}
            className="w-full h-full object-contain"
          />
          {/* View counter */}
          <div className="absolute bottom-4 right-4 font-sans text-navy/30 text-xs tracking-wider">
          ...
          {imageCount > 1 && (
            <>
              <button
                onClick={() => setActiveIndex((i) => (i - 1 + imageCount) % imageCount)}
                className="absolute left-3 top-1/2 -translate-y-1/2 bg-navy/10 hover:bg-navy/20 text-navy/60 hover:text-navy p-2 transition-all"
              ...
              <button
                ...
                className="absolute right-3 top-1/2 -translate-y-1/2 bg-navy/10 hover:bg-navy/20 text-navy/60 hover:text-navy p-2 transition-all"
        ...
        <div
          ...
          role="img"
          aria-label={`${product.name} in ${selectedColor.name} — view ${activeIndex + 1} of ${imageCount}`}
        >
          ...
          {hasRealImages ? (
            colorImages.map((src, i) => (
              <button
                ...
                className={`flex-1 aspect-square overflow-hidden bg-white transition-all duration-200 ${
